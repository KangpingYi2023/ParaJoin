...
namespace VectorJoin
{
    ...
    public:
        ...
        size_t centroids_nb, pq_D, pq_N, pq_M, pq_nbits, code_size;
        std::vector<int> landmark_labels;
        std::vector<std::vector<int>> candidate_entry_points;
        std::vector<float> landmark_dists;
            ...
            do
            {
                ...
                for (size_t j = 1; j <= size; ++j)
                {
                    ...
                    if (neighborId < ql || neighborId > qr)
                        continue;
                    if (std::find(selected_edges.begin(), selected_edges.end(), neighborId) != selected_edges.end())
                        continue;
        ...
        std::vector<tableint> SelectEdgeParallel(int pid, int ql, int qr, int edge_limit)
        {
            TreeNode *cur_node = nullptr, *nxt_node = tree->root;
            std::vector<tableint> selected_edges;
            selected_edges.reserve(edge_limit);
        ...
        std::priority_queue<PFI> TopDown_nodeentries_search(std::vector<TreeNode *> &filterednodes, const void *query_data, int ef, int query_k, int QL, int QR, int edge_limit)
        {
            ...
            // searcher::AtomicBitset<uint64_t> visited_set(max_elements_);

            for (auto u : filterednodes)
            {
                std::uniform_int_distribution<int> u_start(u->lbound, u->rbound);
                int pid = u_start(e); // Random start point
                visited_set.set(pid);
