...
namespace iRangeGraph_multi
{
    ...
    public:
        ...
        iRangeGraph_Search_Multi(std::string edgefilename, DataLoader *store, int M) : storage(store)
        {
            ...
            dim_ = storage->Dim;
            tree = new VectorJoin::SegmentTree(max_elements_);
        ...
        {
            VectorJoin::TreeNode *cur_node = nullptr, *nxt_node = tree->root;
        ...
        std::priority_queue<PFI> TopDown_search(const void *query_data, int ef, int query_k, int QL, int QR, int edge_limit, std::vector<std::pair<int, int>> queryrange, std::vector<VectorJoin::TreeNode *> &filterednodes)
        {
            visited_tag++;
